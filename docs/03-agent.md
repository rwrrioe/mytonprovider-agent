...
| **Agent**    | TON liteserver, ADNL, DHT, ifconfig HTTP, оркестрация цикла     | пишет в Postgres только LT-чекпоинты (для идемподентности) |
...
## Циклы и архитектура агента

Сервис состоит из воркеров 7 типов, которые в свою очередь разбиты на 4 больших юзкейса.

![](/assets/WorkerTypes.svg)
...
подробнее про функционал каждого юзкейса [здесь](./03-agent)
## Идемпотентность
